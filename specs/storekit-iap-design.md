...
|---|---|---|---|
| 包年支持 | ¥6/年 | Auto-Renewable Subscription | `cn.wflixu.Monet.yearly` |
| 终生买断 | ¥18 | Non-Consumable | `cn.wflixu.Monet.lifetime` |
...
在 Xcode 项目中创建 `Monet.storekit` 配置文件，模拟 App Store 环境：

**文件内容（`Monet.storekit`）：**
```
- 添加两个产品：
  - cn.wflixu.Monet.yearly (Auto-Renewable Subscription, ¥6/年)
  - cn.wflixu.Monet.lifetime (Non-Consumable, ¥18)
- 启用 "Use StoreKit Configuration" 后，所有购买走本地模拟
- 无需联网，无需 App Store Connect 配置
```

**在 Xcode 中启用：**
1. 用 Xcode 打开 `Monet.xcodeproj`
2. Product → Scheme → Edit Scheme → Run → Options
3. StoreKit Configuration → 选择 `Monet.storekit`
...
在 `MonetApp.swift` 的 `commands` 中添加：
